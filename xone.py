...
class App(tk.Tk):
    ...
    def protectThread(self):
        ...
        global th_pt
        th_pt = Timer(0.5, self.protectThread)
        th_pt.start()
    ...
    def TaskMonitoring(self):
        ...
        if coolDown > 18:
            global th_update
            th_update =threading.Thread(self.Mf.updateInfo(), daemon=True)
            th_update.start()
            coolDown = 1
        if coolDown: coolDown += 1
        global th_tm
        th_tm =Timer(3, self.TaskMonitoring)
        th_tm.start()
    ...
    def GoToMain(self, FromObject):
        ...
        # 启动多任务管理线程
        threading.Thread(target=self.TaskMonitoring, name="TM", daemon=True).start()
        threading.Thread(target=self.protectThread, name="PT", daemon=True).start()
...
class WelcomeFrame(tk.Frame):
    ...
    def __init__(self, master):
        ...
        th_GoToMain = Timer(2, master.GoToMain, args=(self,))
        th_GoToMain.start()
...
class MainFrame(tk.Frame):
    ...
    def initQaList(self):
        # 左边问题列表区域
        self.frm_left = ttk.Frame(self.panedwin, relief=tk.SUNKEN, padding=0, width=300)
    ...
    def initDetail(self):
        self.frm_right = ttk.Frame(self.panedwin, relief=tk.SUNKEN, width=320)  # 右侧Frame帧用于放置视频区域和控制按钮
    ...
    def onButtonClicked(self, cbname):
        if cbname == '查看线程':
            ...
            logger.info("拾取颜色")
            threading.Thread(target=self.onMouseWatch, name="onMouseWatch", daemon=True).start()
